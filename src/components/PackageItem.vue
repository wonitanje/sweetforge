<template>
  <div :class="{ selected: order.package == _id }" class="item">
    <div class="logo">
      <img @click.stop="openImagePreview" v-lazy="imageName(name)" />
    </div>
    <div class="data">
      <div class="name">{{ name }}</div>
      <div class="capacity">Вместимость: {{ capacity }} гр</div>
      <div class="size">Размеры: {{ size }} см</div>
...
export default {
  ...
  methods: {
    ...
    imageName: name => `${server_url}/image/package/${name.replaceAll(' ', '_').toLowerCase()}.png?scale=3`,
    imageRoot: name => `${server_url}/image/root/${name}.png?scale=1`,

    openImagePreview() {
      this.updateImagePreview({ key: 'package', value: this.name })
